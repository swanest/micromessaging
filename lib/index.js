...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    function handleWaitingRequests(status = 'resolve') {
        memoryPressureTracer.debug('handling waiting requests ( %n ): %s', requestsNeedToWait.bulk.length, status);
        ...
        events.forEach(function (ev, evIndex) {
            ...
            memoryPressureHandler.on(ev, function (mem) {
                ...
                if (ev == memoryPressure.EVENTS.UNDER_PRESSURE) {
                    if (requestsNeedToWait.nackTimeout == void 0) {
                        ...
                        requestsNeedToWait.nackTimeout = setTimeout(()=>{
                            memoryPressureHandler.info('calling handleWaitingRequests(status=reject) due to the reminder');
                            handleWaitingRequests('reject');
                ...
                return retry(function (max, i, insistFn, giveUpFn) {
                    memoryPressureTracer.info("memory: %e, calling prefetch(%c), attempt:%i", ev, prefetchCount, i);
