...
const Service = function Service(name = uuid.v1()) {
    ...
    if (!_.isString(name))
        throw new CustomError("emptyServiceName", "service name must be a non-empty string", 500, "fatal");
    if (name == "_G_")
    ...
    this.close = ()=> {
        return when()
            ...
            }).timeout(1500, new CustomError(CONNECTION_NAME + " closing operation timed out (maybe due to pending items)"));
    };
    ...
    this.connect = (uri = process.env.RABBITMQ_URI || "amqp://localhost") => {
        ...
        settings.queues = [Q_MESSAGES, Q_REQUESTS];
        settings.bindings = [
            {
    ...
    this.emit = (serviceName, route, data, headers = null, opts = null) => {
        if (!_.isString(serviceName) || serviceName == "#")
    ...
    function addRoute(route) {
        routesListeners.push(route);
        rabbot.addQueueBinding(settings.bindings[0].exchange, settings.bindings[0].target, route, CONNECTION_NAME);
    };

    function removeRoute(route) {
        let f = routesListeners.indexOf(route);
        if (f == -1)
            throw new CustomError("routingKeyError", "failed to remove routingKey " + route + ": routesListeners array does not include it", 500, "fatal");
        _.pullAt(routesListeners, [f]);
        if (routesListeners.indexOf(route) == -1)
            rabbot.removeQueueBinding(settings.bindings[0].exchange, settings.bindings[0].target, route, CONNECTION_NAME);
    };
    ...
    this.listen = (route, handler, serviceName = SERVICE_NAME) => {
        ...
        if (serviceName == "*")
            routes.push("public.*." + route, "private." + SERVICE_NAME + "." + route);
        else if (serviceName != SERVICE_NAME)
            routes.push("public._G_." + route, "public." + serviceName + "." + route);
        else
            routes.push("public._G_." + route, "private." + SERVICE_NAME + "." + route, "public." + SERVICE_NAME + "." + route);
        ...
        routes.forEach(function (r) {
            let opts = {};
            opts.connectionName = CONNECTION_NAME;
            opts.queue = Q_MESSAGES.name; //handle messages coming only from this specified queue
            ...
            subscriptions.push(rabbot.handle(opts, _handler));
        });
