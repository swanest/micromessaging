...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    this.close = () => {
        IS_CLOSED = true;
        memoryPressureHandler && memoryPressureHandler.clear(), memoryPressureHandler = null; // stop memory handling !
        if (ONLINE_SINCE == void 0)
            return when();
        return when().then(function () {
            return _THIS.prefetch(0, true); // stop receiving !
        }).delay(500).then(function () {
            rabbot.batchNack();
            _.each(waitingRequests, (req) => {
                req.message.nack();
            });
            ...
            function election() {
                ...
                return _THIS.getStatus(_THIS.name, {expiresAfter: setupOpts.discoverable.electionTimeout}).then(function (st) {
                    ...
                    for (let i = 0; i < st.instances.length; i++) {
                        ...
                        // if already elected in level 1 or level 2, recheck in 5 min and ask to update
    ...
    function publish(exchange, opts) {
        ...
        let ex = when(),
            unRoutableError = new CustomError("unroutableMessage");
    ...
    this.handle = (taskName, handler) => {
        ...
        function _handler(message) {
            message = filter(message);
            let s = when();
            if (requestsNeedToWait) {
                let d = when.defer();
                waitingRequests.push({defer: d, message: message});
                s = d.promise;
            }
    ...
    this.prefetch = function (count, priority) {
        if (IS_CLOSED && !priority)
            return when();
        if (!ONLINE_SINCE)
            throw new CustomError("connectionMissing", "This instance is not connected and therefore cannot perform this action", 500, "fatal");
        let defer = when.defer();
        prefetchCounts[priority ? 'unshift' : 'push']({defer: defer, count: count});
        ...
        [
            {
                ...
                count: 1 // memory-handling prefers prefetch(1) and make it wait instead of prefetch(0) to limit channel errors
            },
            {
            ...
            memoryPressureHandler.on(ev.event, function (mem) {
                if (ev.event == memoryPressure.EVENTS.UNDER_PRESSURE)
                    requestsNeedToWait = true;
                else {
                    requestsNeedToWait = false;
                    _.each(waitingRequests, (wR) => {
                        wR.defer.resolve();
                    });
                    waitingRequests = [];
                }
                let prefetchCount = ev.event == memoryPressure.EVENTS.PRESSURE_RELEASED ? lastPrefetchCount : ev.count;
    ...
    this.waitForService = (serviceName, opts = {}) => {
        ...
        return poll(function () {
            ...
            if (!status.isReady)
                throw new CustomError("unready", "Waiting for service to be ready timed out", 500, "fatal", {status: _.extend(status, {attempts: attempts})});
