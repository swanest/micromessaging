...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    _.defaultsDeep(setupOpts, {
        ...
        config: {
            ...
            Q_REQUESTS: {
                ...
                expires: 10 * 60 * 1000, //deleted if unused during 10min
                autoDelete: true //this deletes the queue when the amount of consumers goes from 1 to zero
            },
            Q_DEAD_REQUESTS: {
                durable: false,
    ...
    if (setupOpts.memoryPressureHandled) {
        memoryPressureHandler = require("memory-pressure").new(CONNECTION_NAME, setupOpts.memoryPressureHandled);
        [{event: "underPressure", count: 0}, {event: "pressureReleased", count: null}].forEach(function (ev) {
            memoryPressureHandler.on(ev.event, (mem) => {
                tracer.info("memory: %e, calling prefetch(%c)", ev.event, ev.count == null ? lastPrefetchCount : ev.count, mem);
                _this.prefetch(ev.count == null ? lastPrefetchCount : ev.count).then(function () {
                    tracer.debug("prefetching succeeded");
                }, function (err) {
                    tracer.error("prefetching failed", err);
                }).finally(function () {
                    memoryPressureHandler.ack();
                });
            });
        });
    }
    ...
    let prefetch = (count) => {
        ...
        tracer.log("PREFETCHING", count, "consumers", _.keys(queueChann.item.consumers), "tag", queueChann.tag);
