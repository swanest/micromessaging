...
const filter = (message) => {
    ...
    if (!_.get(message.properties.headers, "_mms_no_ack")) {
        ...
        if (message.reject && msg.type != "request")
    ...
    if (!_.get(message.properties.headers, "_mms_no_reply") && message.reply) {
        ...
        let originalReplyFunc = message.reply,
            replied = false,
            replyFunc = (m, headers) => {
                if (replied)
...
rabbot.onReturned((message) => {
    ...
    return inst.__emit("unroutableMessage", filter(message));
});
...
rabbot.onUnhandled((message) => {
    ...
    return inst.__emit("unhandledMessage", filter(message));
});
...
//todo:check on puisse pas reply dans un then/progress de request, check les fields dans progress/then
    ...
    this.listen = (route, handler, serviceName = SERVICE_NAME) => {
        ...
        function _handler(message) {
            return handler.call(this, filter(message));
        };
