...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    this.setMemoryHandling = function (memoryHandleParams) {
        ...
        function dynamicPrefetch() {
            ...
            lastPrefetchCount = DYNAMIC_SIMULTANEOUS_REQUESTS += DYNAMIC_PREFETCH_STEP;
            if (DYNAMIC_SIMULTANEOUS_REQUESTS >= setupOpts.simultaneousRequests) { // Cap the increase in prefetch(..)
                DYNAMIC_SIMULTANEOUS_REQUESTS = setupOpts.simultaneousRequests;
                return setTimeout(dynamicPrefetch, DYNAMIC_PREFETCH_INTERVAL);
            }
