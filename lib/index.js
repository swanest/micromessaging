...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    if (setupOpts.memoryPressureHandled) {
        ...
        [
            ...
            memoryPressureHandler.on(ev.event, function (mem) {
                tracer.info("memory: %e, calling prefetch(%c)", ev.event, ev.count == null ? lastPrefetchCount : ev.count);
                return _this.prefetch(ev.count == null ? lastPrefetchCount : ev.count).then(function () {
                    tracer.debug("prefetching succeeded");
                }, function (err) {
                    tracer.error("prefetching failed", err);
                }).finally(function () {
                    mem.ack();
                });
            });
    ...
    let prefetch = (count) => {
        let connectionObj = _.get(rabbot.connections, CONNECTION_NAME),
            ...
            queueChannel = _.get(connectionChannels, "queue:" + Q_REQUESTS.name),
            lastQueueChannel = _.get(queueChannel, "lastQueue.channel");
        if (connectionObj == void 0)
            return when.reject(new CustomError("connectionMissing", "connection is missing. prefetch() has maybe been called on a close service.", 500, "fatal"));
        if (lastQueueChannel == void 0)
            return when.reject(new CustomError("channelNotReady", "queueChannel is not ready", 500, "fatal"));
        tracer.log("PREFETCHING", count, "consumers", _.keys(lastQueueChannel.item.consumers), "tag", lastQueueChannel.tag);
        if (_.keys(lastQueueChannel.item.consumers).length == 0 && count == 0) {
        ...
        if (!subscribing)
            return when();
        return connection.getChannel("ctrl:prefetch:" + UNIQUE_ID).then(function (controlChannel) {
            //Check queue
            return controlChannel.checkQueue(Q_REQUESTS.name).then(_.noop, function () {
                ...
                return rabbot.deleteQueue(Q_REQUESTS.name, CONNECTION_NAME).then(function () {
                    connection.removeChannel(["queue", Q_REQUESTS.name].join(":"));
                    //Recreate a channel, a queue and add binding...
                    return connection.getChannel(["queue", Q_REQUESTS.name].join(":")).then(function (queueChannel) {
            ...
            //Here we can be sure the queue exists ...So we can either stop flowing messages or change prefetch()
            if (count == 0)
                ...
                return rabbot.startSubscription(Q_REQUESTS.name, Q_REQUESTS.exclusive, CONNECTION_NAME).then(function () {
                    let queueChannel = _.get(rabbot.connections[CONNECTION_NAME].channels[["queue", Q_REQUESTS.name].join(":")], "lastQueue.channel");
                    return queueChannel.prefetch(count, true);
                });
        }).then(function () {
            let queueChann = _.get(rabbot.connections[CONNECTION_NAME].channels["queue:" + Q_REQUESTS.name], "lastQueue.channel")
            tracer.log("PREFETCHED", count, "consumers", _.keys(queueChann.item.consumers), "tag", queueChann.tag);
        });
    };

    let isPrefetchRunning = false, prefetchCounts = [];
