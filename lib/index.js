...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    this.close = () => {
        ...
        return when().then(function () {
            ...
            rabbot.batchNack(); // nack all running requests !
            handleWaitingRequests(false); // nack all waiting requests !
    ...
    function handleWaitingRequests(status) {
        requestsNeedToWait = false;
        _.each(waitingRequests, (req) => {
            if (!status)
                req.message.nack();
            req.defer[status ? 'resolve' : 'reject']();
        });
        waitingRequests = [];
    };
    ...
    this.setMemoryHandling = function (memoryHandleParams) {
        if (memoryHandleParams && memoryPressureHandler)
            return true;
        if (!memoryHandleParams && !memoryPressureHandler)
            return true;
        if (!memoryHandleParams) {
            handleWaitingRequests(false), memoryPressureHandler.clear(), memoryPressureHandler = null;
            return true;
        }
