...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    if (!setupOpts.discoverable)
        setupOpts.discoverable = false;
    ...
    _.defaultsDeep(setupOpts, {
        discoverable: {
            intervalCheck: 10 * 60 * 1000, //each 10', we need to check whether there's still an elected instance online, otherwise we elect one
            electionTimeout: 300 //time to wait (in ms) after the last online.res event to define the elected instance
        }
    });
    ...
    let hasListeners = false,
        ...
        Q_MESSAGES = {
            ...
            exclusive: true, //this queue is unique to this instance
        ...
        Q_RESPONSES = {
            ...
            limit: Math.pow(2, 16) - 1, //consumer prefetch limit
            queueLimit: Math.pow(2, 32) - 1, //max messages a queue can hold
            exclusive: true, //this queue is unique to this instance, no one else is allowed to consume from
            autoDelete: true, //deleted when the amount of consumers goes zero
            noAck: true, //no need to ack,nack,reject...
    ...
    let autoElection = () => {
        ...
        when()
            .then(() => {
                return this.listen.call({noCheck: true}, "_mms_online.res", (msg) => {
                    if (_.find(this.replications, _.pick(msg.body, "uniqueID")) != void 0)
                        return;
                ...
                return this.listen.call({noCheck: true}, "_mms_online.req", () => {
    ...
    //Start consuming
    this.subscribe = () => {
        this.subscribe = () => { //For next calls todo:test it
    ...
    function addRoutingDef(def) {
        ...
        return rabbot.addQueueBinding(EXCHANGE_MESSAGES.name, def.queueName, def.routingKey, CONNECTION_NAME);
    };
    ...
    function removeRoutingDef(def) {
        ...
        if (_.findIndex(routesListeners, def) == -1) {
            if (def.queueName == Q_MESSAGES.name) //not shared queue, unique to the service instance, so we can remove the binding-key
                return rabbot.removeQueueBinding(EXCHANGE_MESSAGES.name, def.queueName, def.routingKey, CONNECTION_NAME);
        }
        return when();
    };
    ...
    this.listen = function (route, handler, serviceName = SERVICE_NAME) {
        ...
        let queueName = this.queueName || Q_MESSAGES.name, routes = [], promises = [];

        if (serviceName == "*")
            routes.push(
                {routingKey: "public.*." + route, queueName: queueName},
                {routingKey: "private." + SERVICE_NAME + "." + route, queueName: queueName});
        else if (serviceName != SERVICE_NAME)
            routes.push(
                {routingKey: "public._G_." + route, queueName: queueName},
                {routingKey: "public." + serviceName + "." + route, queueName: queueName});
        else
            routes.push(
                {routingKey: "public._G_." + route, queueName: queueName},
                {routingKey: "private." + SERVICE_NAME + "." + route, queueName: queueName},
                {routingKey: "public." + SERVICE_NAME + "." + route, queueName: queueName});

        routes.forEach(function (def) {
            promises.push(addRoutingDef({routingKey: def.routingKey, queueName: def.queueName}));
        });
    ...
    //let needToRestartSubscription = false;
    //this.prefetch = (count) => { //todo:test with auto-delete and not auto-delete
    //    console.log("bebe");
    //    if (!subscribing)
    //        return when();
    //    let queueChannel = rabbot.connections[CONNECTION_NAME].channels["queue:" + Q_REQUESTS.name].lastQueue.channel;
    //    tracer.log(queueChannel.tag, "" + count);
    //    let p = when();
    //    if (count == 0 && !needToRestartSubscription) {
    //        console.log("removing...");
    //        needToRestartSubscription = true, p = rabbot.stopSubscription(Q_REQUESTS.name, CONNECTION_NAME).catch(function (err) {
    //            tracer.error("Could not stop subscription");
    //        });
    //    }
    //    else if (needToRestartSubscription) { //we need to recreate the queue as it is in autoDelete mode
    //        needToRestartSubscription = false;
    //        console.log("adding...");
    //
    //        //checkQueue
    //        rabbot.addQueue(Q_REQUESTS.name, Q_REQUESTS, CONNECTION_NAME).then(function(q){
    //            return rabbot.bindQueue(EXCHANGE_REQUESTS.name, Q_REQUESTS.name, ["task"], CONNECTION_NAME);
    //        });
    //
    //        p = rabbot.addQueue(Q_REQUESTS.name, Q_REQUESTS, CONNECTION_NAME).then(function (q) {
    //            return rabbot.startSubscription(Q_REQUESTS.name, Q_REQUESTS.exclusive, CONNECTION_NAME).then(function () {
    //                tracer.log("queue recretaded");
    //            }).catch(function (err) {
    //                tracer.error(err);
    //                throw err;
    //            });
    ...
    //        console.log("end", needToRestartSubscription);
    //        if (count != 0)
    ...
    this.getRequestsReport = (serviceName) => { //todo:test it
        let queueChannel = rabbot.connections[CONNECTION_NAME].channels["queue:" + Q_REQUESTS.name].lastQueue.channel;
