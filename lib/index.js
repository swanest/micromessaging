...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    function handleWaitingRequests(status = 'resolve', nFirst) {
        ...
        if(nFirst == void 0)
            nFirst = requestsNeedToWait.bulk.length;

        memoryPressureTracer.debug('handling waiting requests ( %n / %t ): %s', nFirst, requestsNeedToWait.bulk.length, status);
    ...
    this.setMemoryHandling = function (memoryHandleParams) {
        ...
        if (memoryPressureHandler) // cancel previous memory handler if any
            memoryPressureHandler.clear(), memoryPressureHandler = null, _THIS.memoryPressureHandler = memoryPressureHandler;
        ...
        events.forEach(function (ev, evIndex) {
            ...
            memoryPressureHandler.on(ev, function (mem) {
                ...
                // let memoryUsage = _.last(mem.memoryUsageHistory);
                ...
                if (ev == memoryPressure.EVENTS.UNDER_PRESSURE) {
                    ...
                    if(requestsNeedToWait.bulk.length > 1){ // bulk by itself can be already memory-demanding, nack all except 1
                    ...
                    if (requestsNeedToWait.nackTimeout == void 0) {
                        ...
                        requestsNeedToWait.nackTimeout = setTimeout(() => {
                            memoryPressureHandler.info('calling handleWaitingRequests(status=reject) due to the reminder: going to approximately release %s mb', sizeof(requestsNeedToWait.bulk) / 1e6);
                            handleWaitingRequests('reject');
