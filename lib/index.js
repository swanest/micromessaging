...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    this.subscribe = (setAsReady = true) => {
        ...
        return when().then(function () {
            ...
            return _THIS.listen.call({noCheck: true}, "_mms_status.req", (msg) => {
    ...
    this.getStatus = (serviceName, opts = {}) => { //if isElected == null, we do not care, if isElected=bool, we specify
        if (!SUBSCRIBING)
            throw new CustomError("subscriptionMissing", "This instance is not subscribing and therefore cannot perform this action", 500, "fatal");
        ...
        let replyRoute = "_mms_status.res." + uuid.v4(),
            instances = [],
            listener = _THIS.listen.call({noCheck: true}, replyRoute, (msg) => {
