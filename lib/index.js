...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    const _THIS = this,
        ...
        DYNAMIC_PREFETCH_INTERVAL = 300,
        DYNAMIC_PREFETCH_DELAY_AFTER_UNDER_PRESSURE = 2000,
        DYNAMIC_PREFETCH_STEP = 10;
    ...
    let ONLINE_SINCE = null,
        ...
        SUBSCRIBING = false,
        IS_READY = false,
        LAST_HANDLED_REQUEST; // Date when there was a request handled

    //Fill in default configuration
    setupOpts = defaultConf(SERVICE_NAME, CONNECTION_NAME, setupOpts);

    let DYNAMIC_SIMULTANEOUS_REQUESTS = setupOpts.initSimultaneousRequests;
    ...
    this.subscribe = (setAsReady = true) => {
        ...
        return when().then(function () {
            ...
            return _THIS.prefetch.call({ initialPrefetch: true }, DYNAMIC_SIMULTANEOUS_REQUESTS);
        }).catch(function (err) {
    ...
    function publish(exchange, opts) {
        ...
        return ex.then(function () {
            return rabbot[opts.routingKey == 'task' ? 'request' : 'publish'](exchange.name, opts).catch(function (e) {
                throw new CustomError(e.message == 'unroutable' ? 'unroutableMessage' : 'publishError', e, {
    ...
    this.listen = function (route, handler, serviceName = SERVICE_NAME) {
        ...
        if (queueName == Q_DEAD_REQUESTS.name)
            ...
            routes.push(
                ...
                {
                    exchangeName: exchangeName,
                    routingKey: 'public.' + serviceName + '.' + route,
                    queueName: queueName
                });
        else
            routes.push(
                ...
                {
                    exchangeName: exchangeName,
                    routingKey: 'public.' + SERVICE_NAME + '.' + route,
                    queueName: queueName
                });
    ...
    let // variables touched by memory handling process and used in .handle()
        ...
        prefetchCounts = [],
        lastPrefetchCount = DYNAMIC_SIMULTANEOUS_REQUESTS,
        prefetchManual = false,
        maxPrefetchBound = setupOpts.simultaneousRequests;
    ...
    this.prefetch = function (count) {
        ...
        if (!ONLINE_SINCE)
            throw new CustomError('connectionMissing', 'This instance is not connected and therefore cannot perform this action', 500, 'fatal');
        if (count == void 0) {
            return lastPrefetchCount;
        }
        if (count > maxPrefetchBound) {
            tracer.log(`capped prefetch to ${maxPrefetchBound} instead of ${count} because it is higher that maxPrefetchBound`);
            count = maxPrefetchBound;
        }
        if (!this.doNotRemember) {
            lastPrefetchCount = count;
        }
        if (!this.doNotRemember && !this.initialPrefetch) {
            prefetchManual = true;
        }
    ...
    this.maxPrefetch = function (count) {
        if (count == void 0) {
            return maxPrefetchBound;
        }
        maxPrefetchBound = count;
        if (lastPrefetchCount > maxPrefetchBound) {
            return _THIS.prefetch.call({ doNotRemember: true }, maxPrefetchBound);
        }
    ...
    this.setMemoryHandling = function (memoryHandleParams) {
        ...
        let IS_UNDER_PRESSURE = false,
            LAST_UNDER_PRESSURE,
            LAST_PRESSURE_RELEASED,
            hasToDecreaseOnce = false;

        function dynamicPrefetch() {
            if (prefetchManual) {
                return;
            }
            if (IS_UNDER_PRESSURE && (!LAST_PRESSURE_RELEASED || LAST_PRESSURE_RELEASED > LAST_UNDER_PRESSURE) && hasToDecreaseOnce) {
                /**
                 * The goal is is to find the right spot bellow the under pressure event by decreasing once just after
                 * getting an under_pressure event.
                 */
                hasToDecreaseOnce = false;
                lastPrefetchCount = DYNAMIC_SIMULTANEOUS_REQUESTS -= (2 * DYNAMIC_PREFETCH_STEP);
                if (DYNAMIC_SIMULTANEOUS_REQUESTS <= 0) {
                    lastPrefetchCount = DYNAMIC_SIMULTANEOUS_REQUESTS = DYNAMIC_PREFETCH_STEP;
                }
            }
            if (!LAST_HANDLED_REQUEST) { // Meaning we never handled anything so we don't increase...
                return setTimeout(dynamicPrefetch, DYNAMIC_PREFETCH_INTERVAL);
            }
            const sinceHandled = new Date() - LAST_HANDLED_REQUEST;
            let isHandlingRequests = false;
            if (sinceHandled < DYNAMIC_PREFETCH_INTERVAL) {
                isHandlingRequests = true;
            }
            if (IS_UNDER_PRESSURE || !isHandlingRequests || // No decision while under pressure neither while NOT handling requests
                (LAST_PRESSURE_RELEASED && new Date() - LAST_PRESSURE_RELEASED < DYNAMIC_PREFETCH_DELAY_AFTER_UNDER_PRESSURE) // We avoid increasing if it was under pressure in the last 5seconds
            ) {
                return setTimeout(dynamicPrefetch, DYNAMIC_PREFETCH_INTERVAL);
            }
            if (LAST_PRESSURE_RELEASED && LAST_UNDER_PRESSURE &&
                LAST_PRESSURE_RELEASED < LAST_UNDER_PRESSURE &&
                LAST_PRESSURE_RELEASED - LAST_UNDER_PRESSURE < DYNAMIC_PREFETCH_DELAY_AFTER_UNDER_PRESSURE
            ) {
                lastPrefetchCount = DYNAMIC_SIMULTANEOUS_REQUESTS -= DYNAMIC_PREFETCH_STEP;
                return setTimeout(dynamicPrefetch, DYNAMIC_PREFETCH_INTERVAL);
            }
            lastPrefetchCount = DYNAMIC_SIMULTANEOUS_REQUESTS += DYNAMIC_PREFETCH_STEP;
            if (DYNAMIC_SIMULTANEOUS_REQUESTS >= maxPrefetchBound) { // Cap the increase in prefetch(..)
                DYNAMIC_SIMULTANEOUS_REQUESTS = maxPrefetchBound;
                return setTimeout(dynamicPrefetch, DYNAMIC_PREFETCH_INTERVAL);
            }

            return _THIS.prefetch.call({ doNotRemember: true }, DYNAMIC_SIMULTANEOUS_REQUESTS)
                .then(function () {
                    tracer.info('calling prefetch(%c)', DYNAMIC_SIMULTANEOUS_REQUESTS);
                    setTimeout(dynamicPrefetch, DYNAMIC_PREFETCH_INTERVAL);
                })
                .catch(function (err) {
                    tracer.error('prefetch(%c) failed', DYNAMIC_SIMULTANEOUS_REQUESTS, err);
                    throw err;
                });
        }
