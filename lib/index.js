...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    this.prefetch = (count) => {
        lastPrefetchCount = count != 0 ? count : null;
        if (!subscribing)
            return when();
        return connection.getChannel("ctrl:prefetch:" + UNIQUE_ID).then(function (channel) {
            ...
            return channel.checkQueue(Q_REQUESTS.name).then(function () {
                //Exists ?
                if (count == 0) {
                    return rabbot.stopSubscription(Q_REQUESTS.name, CONNECTION_NAME).catch(function () {
                        //Retry
                ...
                return when().then(function () {
                    ...
                    rabbot.startSubscription(Q_REQUESTS.name, Q_REQUESTS.exclusive, CONNECTION_NAME).then(function () {
