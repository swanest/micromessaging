...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    let needToRestartSubscription = false, lastPrefetchCount = null;
    ...
    this.prefetch = (count) => {
        ...
        let queueChannel = _.get(rabbot.connections[CONNECTION_NAME].channels["queue:" + Q_REQUESTS.name], "lastQueue.channel"),
            p = when();
        if (count == 0 && !needToRestartSubscription && _.keys(queueChannel.item.consumers).length) {
            needToRestartSubscription = true, p = rabbot.stopSubscription(Q_REQUESTS.name, CONNECTION_NAME);
        }
        else if (needToRestartSubscription) {
            needToRestartSubscription = false;
            p = queueChannel.checkQueue(Q_REQUESTS.name).then(_.noop, function () {
                return rabbot.addQueue(Q_REQUESTS.name, Q_REQUESTS, CONNECTION_NAME).then(function (q) {
    ...
    if (setupOpts.memoryPressureHandled) {
        let memory = require("memory-pressure").new(this.__connectionName, setupOpts.memoryPressureHandled), lastPrefetchCountCopy = lastPrefetchCount;
        memory.on("underPressure", (mem) => {
            lastPrefetchCountCopy = lastPrefetchCount;
            ...
            lastPrefetchCount = lastPrefetchCountCopy;
        });
        memory.on("pressureReleased", () => {
            this.prefetch(lastPrefetchCount);
        });
    }
