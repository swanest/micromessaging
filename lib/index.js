...
rabbot.onReturned((message) => {
    ...
    when().then(function () {
        if (!_.get(message, "properties.headers._mms_internal"))
...
const Service = function Service(name = uuid.v1(), setupOpts = {}) {
    ...
    this.connect = (uri = process.env.RABBITMQ_URI || "amqp://localhost") => {
        ...
        return rabbot.configure(settings).then(() => {
            ...
            return _THIS.listen.call({
                ...
                }, {_mms_internal: true}).catch(_.noop);
            }).promise
        }).then(function () {
    ...
    let autoElection = () => {
        let autoElect = () => {
                ...
                if (_.first(this.replications).uniqueID == UNIQUE_ID && !this.isElected) {
                    ...
                    this.__emit("elected");
                    if (SUBSCRIBING)
                        this.__emit("electedAndSubscribing");
                    if (IS_READY)
                        this.__emit("electedAndReady");
                }
            },
        ...
        when()
            .then(() => {
                return this.listen.call({
                    noCheck: true,
                    queueName: Q_INTERNAL_MESSAGES.name
                }, "_mms_online.res", (msg) => { //Getting all my neighbours presences
                    ...
                    this.replications && this.replications.push(msg.body);
            ...
            return this.listen.call({
                noCheck: true,
                queueName: Q_INTERNAL_MESSAGES.name
            }, "_mms_online.req", () => { //Listening on request to reveal myself
                clearTimeout(recheckTimeout);
                recheckTimeout = setTimeout(() => {
                    try {
                        this.privately.emit.call({noCheck: true}, this.name, "_mms_online.req", null, {_mms_internal: true}); //Ask all instances of service to reveal
                    } catch (e) {
            ...
            this.privately.emit.call({noCheck: true}, this.name, "_mms_online.req", null, {_mms_internal: true}); //Ask all instances of service (including myself) to reveal
        });
    };
    ...
    this.subscribe = (setAsReady = true) => {
        ...
        }).timeout(15000, new CustomError('subscriptionTimeout', 'subscription timed out')).catch(function (err) {
