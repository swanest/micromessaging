    ...
    this.prefetch = (count) => {
        ...
        return connection.getChannel("ctrl:prefetch:" + UNIQUE_ID).then(function (channel) {
            ...
            return channel.checkQueue(Q_REQUESTS.name).then(function () {
                //Exists ?
                if (count == 0) return rabbot.stopSubscription(Q_REQUESTS.name, CONNECTION_NAME).catch(function () {
                    //Retry
                    return when().delay(2000).then(function () {
                ...
                //Does not exist ?
                if (count == 0) throw new CustomError("queueNotFound", "tried to unsubscribe from a missing queue", 500, "fatal");
