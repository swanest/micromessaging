...
const RESERVED_HEADERS = [
    ...
    "sequence_end",
    "_rabbot_idm",
    /^_mms_*/ // _mms_no_reply, _mms_no_ack, _mms_type
];

const RESERVED_ROUTES = [
    /^_mms_*/, // _mms_status.req
];

const cleanHeaders = (h) => {
    if (_.isPlainObject(h)) {
        _.keys(h).forEach((kh) => {
            RESERVED_HEADERS.forEach((rh) => {
                if (kh.match(rh) != void 0)
                    _.unset(h, kh);
            });
        });
    }
...
const checkHeaders = (h) => {
    ...
    if (_.isPlainObject(h)) {
        _.keys(h).forEach((kh) => {
            RESERVED_HEADERS.forEach((rh) => {
                if (kh.match(rh) != void 0)
                    throw new CustomError("unAllowedHeader", "h." + kh + " is reserved", 500, "fatal");
            });
        });
    }
    return h || {};
...
const checkRoute = (r) => {
    ...
    RESERVED_ROUTES.forEach((rr) => {
        if (r.match(rr) != void 0)
