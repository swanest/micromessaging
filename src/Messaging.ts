...
export class Messaging {
     ...
     */
    public get lastMessageDate(): Date {
    ...
    private async _assertParallelBoundaries() {
        ...
        if (this._maxParallelism > 0) {
            ...
            if (this._maxParallelism > 0 && maxParallelismPerConsumer < 1) {
                // To avoid prefetch(0) on all channel and block everything.
        ...
        // Check that the parallelism quota hasn't changed while
        // applying the last params
        if (this._maxParallelism !== this._lastAppliedParallelism.value) {
            this._assertParallelBoundaries().catch(e => this.reportError(e));
        } else if (this._waitParallelismAsserted != null) {
    ...
    private async _assertReplyQueue() {
        ...
        if (this._replyQueueAssertionPromise != null) {
            await this._replyQueueAssertionPromise;
        }
    }

    /**
     * Asserts a route identified by a name into existence. Called
     * after declaration of a route. Idempotent.
    ...
    private async _assertRoute(routeAlias: string) {
        ...
        if (this._isConnected && !this._isClosing && !this._isClosed) {
            ...
            route.consume = async () => {
                ...
                if (route.consumerTag == null && route.consumerWaiter == null) {
                    ...
                    if (route.subjectToQuota && route.maxParallelism > 0) {
                        ...
                        if (this._isClosing) {
                            // Because of the await just above this
                            // line might try to execute on a closing
                            // connection.
                            return;
                        }
            ...
            switch (route.type) {
                case 'rpc':
                    ...
                    // After 24h the queue gets deleted if not used.
                    queueOptions.expires = 24 * 60 * 60 * 1000;
    ...
    private async _createChannel(): Promise<Channel> {
        ...
        chan.on('return', msg => {
            ...
            if (this._awaitingReply.has(msg.properties.correlationId)) {
                ...
                this._unregisterResponseAwait(msg.properties.correlationId);
            } else {
        ...
        // The 'close' hook is set in _assertChannel, since it is more
    ...
    private _messageHandler(originalMessage: AMessage, route: Route) {
        ...
        if (this._serviceOptions.enableQos && route.subjectToQuota && (route.options == null || route.options.maxParallel == null)) {
        ...
        if (route.options && route.options.maxParallelBytes != null) {
        ...
        if (m.isAnswer()) {
            ...
            if (m.isError()) {
                ...
                this._unregisterResponseAwait(m.correlationId());
                return;
            }
            if (m.isStream()) {
                if (defMess.accumulator == null) {
                    defMess.accumulator = [];
                }
                if (typeof defMess.streamHandler !== 'function') {
                    ...
                    // as there is a stream handler restart the
                    // timeout timer on the request.
                    this._tapResponseAwaitTimer(m.correlationId());
    ...
    private _stopListen(routeName: string): (options?: ReturnHandlerStopOpts) => Promise<void> {
        return async ({deleteQueue = true}: ReturnHandlerStopOpts = {}) => {
            ...
            // We do this separately because we could get PRECONDITION
            // errors that we want to ignore.
            if (!deleteQueue) {
                return;
            }
            try {
                const channelThatCanError = await this._connection.createChannel();
                channelThatCanError.on('error', () => undefined);
                ...
                if (!/PRECONDITION/.test(e.message)) {
                    // Let's not swallow too much, just what we are sure we need to.
                    this.reportError(new CustomError('Error while stop listening', e));
                }
    ...
    private _tryDispatchMessage(deferredMessage: ReplyAwaiter, message: Message): boolean {
        ...
        if (message.isStreamEnd()) {
            ...
            this._unregisterResponseAwait(message.correlationId());
            return true;
        }
    ...
    private _tapResponseAwaitTimer(correlationId: string) {
        if (this._awaitingReply.has(correlationId)) {
            ...
            if (corr.timer) {
                ...
                corr.timer = setTimeout(() => {
                    ...
                    this._unregisterResponseAwait(correlationId);
                }, corr.timerMs);
            }
        }
    }

    private _unregisterResponseAwait(correlationId: string) {
        if (this._awaitingReply.has(correlationId)) {
            const corr = this._awaitingReply.get(correlationId);
            if (corr.timer) {
                clearTimeout(corr.timer);
    ...
    //////////////////////////////////////////////
    // Public methods and exposed functionality //
    //////////////////////////////////////////////
    ...
    public async close(deleteAllQueues: boolean = false, force: boolean = false) {
        ...
        // Until we get acks for each, we can still receive messages.
        await Promise.all(cancelConsuming);
        this._routes.forEach(route => {
    ...
    public async assertLeader(): Promise<void> {
        this._assertConnected();
        const channel = await this._assertChannel('__election', true);
        ...
        try {
            ...
            // Possible cases:
            // 1.- The exclusive queue is already taken. RESOURCE_LOCKED error
            // 2.- Channel is closing
            // 3.- Channel is closed

            // This is wanted behaviour, therefore swallow the exception
        }
    }
    ...
    public async getStatus(targetService: string = this._serviceName): Promise<Status> {
        ...
        const hasReadyMembers = members.some(m => m.isReady != null);
        return {
            hasMaster,
            hasReadyMembers,
            members: members,
    ...
    public setMaxParallelism(value: number) {
        ...
        if (this._waitParallelismAsserted == null) {
            this._assertParallelBoundaries().catch(e => this.reportError(e));
        }
    }
    ...
    public async setQosMaxParallelism(value?: number) {
        ...
        this._logger.debug(`Should wait? ${this._waitParallelismAsserted != null}`);
        if (this._waitParallelismAsserted == null) {
            await this._assertParallelBoundaries();
        } else {
    ...
    public async emit<T = {}>(target: string,
        ...
        if (ret !== true && this._bufferFull == null) {
     ...
     * @param target The target on which you want to listen by default
     * it's the serviceName but it can be the name of a commonly
     * agreed exchange
     * @param route The route on which to listen
     * @param listener The callback function that will be called each
     * time a message arrives on that route
     * @return resolves when listeners are fully asserted and returns
     * an object which has a "stop" function.
     */
    ...
    /**
     * See the docs of the two other overloaded methods. This is the
    ...
    public async listen(routeOrTarget: string, routeOrListener: any, listenerOrOptions?: any, opt?: ListenerOptions): Promise<ReturnHandler> {
        ...
        if (this._routes.get(routeAlias) != null) {
            throw new CustomError(`A listener for ${target != null ? '' : target + ':'}${route} is already defined.`);
        }
        this._routes.set(routeAlias, {
    ...
    /**
     * Worker queue implementation. By default we always want an
     * acknowledgment to a sent task (for backwards compatibility).
     *
     * @param {string} targetService The name of the service that will
     * have to handle the request
     * @param {string} route A routing key to the handler in the
     * targetService
     * @param messageBody The message to send.
     * @param messageHeaders Additional headers. If idRequest is not
     * provided, will auto-generate one.
     * @returns A promise that resolves once the message has been sent
     * or a proxied request
     */
